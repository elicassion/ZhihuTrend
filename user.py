...
class UserSpider(scrapy.Spider):
    ...
    def parseAnswer(self, response):
        ...
        info = response.css(".ProfileHeader-detailItem")
        residence = ''
        field = ''
        career = ''
        education = ''
        for index, item in enumerate(info):
        	label = item.css(".ProfileHeader-detailLabel::text")[0].extract()
        	if label == u'居住地':
        		residence = item.css(".ProfileHeader-detailValue::text").extract()

	        elif label == u'所在行业':
	        	field = item.css(".ProfileHeader-detailValue::text").extract()

	        elif label == u'职业经历':
	        	careers = item.css(".ProfileHeader-field::text")
	        	for i, c in enumerate(careers):
	        		if i != 0:
	        			career = career + '|'
	        		career = career + c.extract()

	        elif label == u'教育经历':
	        	educations = item.css(".ProfileHeader-field::text")
	        	for i, e in enumerate(educations):
	        		if i != 0:
	        			education = education + '|'
	        		education = education + e.extract()
        ...
        db.SpiderUserInsert(id, name, description, profession, sex, answer, share, question, collection,
                            ...
                            residence, field, career, education)
